...
Runs Feedly CTI pull and OpenCTI pull concurrently every
``SYNC_INTERVAL_MINUTES`` (default 30).  Each cycle uses a lookback
window of ``SYNC_LOOKBACK_MINUTES`` (default 60, i.e. 1 hour) so
there's overlap to catch anything missed.  Filesystem scan is
intentionally excluded — that remains manual.
...
async def run_sync_loop(
    ...
    """Run Feedly + OpenCTI sync concurrently in a loop every ``sync_interval_minutes``."""
            ...
            async def _sync_feedly():
                for index_name in indices:
                    try:
                        ...
                        logger.exception(
                            "Scheduled Feedly sync failed for index %s", index_name
                        )

            tasks.append(_sync_feedly())
        elif (
            settings.elastic_connector_hosts_list
            and not settings.elastic_connector_enabled
        ):
            logger.debug(
                "Scheduled Feedly sync disabled by ELASTIC_CONNECTOR_ENABLED=0"
            )
